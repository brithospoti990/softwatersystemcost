...
fail = 0
pages = [p for p in glob.glob("**/*.html", recursive=True) if not p.startswith("qa/") and not re.match(r"^google[0-9a-f]+\.html$", p)]  # search-engine verification files are not pages
...
# JSON-LD validity site-wide (v4.38 guard; ld() takes a pre-dumped string — a raw dict ships python repr)
import json as _json, re as _re, os as _os
for _root, _dirs, _files in _os.walk("."):
    if "index.html" in _files and "/.git" not in _root:
        _src = open(_os.path.join(_root, "index.html"), encoding="utf-8", errors="replace").read()
        for _blk in _re.findall(r'<script type="application/ld\+json">(.*?)</script>', _src, _re.S):
            try:
                _json.loads(_blk)
            except Exception:
                fail += 1; print(f"FAIL invalid JSON-LD in {_root}/index.html: {_blk[:70]}")
