...
// softwatersystemcost.com · spec §4.5
...
/** 200ms count-up on the FINAL figure only (spec §2.4). Calls render(text) each frame.
    Instant when prefers-reduced-motion. Returns a cancel function. */
...
/** ZIP → average grains-per-gallon hardness. Static stub keyed on first ZIP digit;
    production swaps this for the full static JSON lookup (no API). */
const ZIP_GPG = { 0: 6, 1: 7, 2: 8, 3: 11, 4: 12, 5: 14, 6: 18, 7: 16, 8: 17, 9: 10 };
export function hardnessForZip(zip) {
  const d = String(zip || '').trim()[0];
  return /[0-9]/.test(d || '') ? ZIP_GPG[+d] : null;
}
